...
class Condition : noncopyable {
 public:
  explicit Condition(MutexLock &_mutex) 
    : mutex(_mutex) 
  {
    MCHECK(pthread_cond_init(&cond, NULL));
  }
  ...
  void wait() 
  { 
    MCHECK(pthread_cond_wait(&cond, mutex.getPthreadMutex())); 
  }//封装pthread_cond_wait
  ...
  bool waitForSeconds(int seconds) 
  {
    ...
    return ETIMEDOUT == pthread_cond_timedwait(&cond, mutex.getPthreadMutex(), &abstime);
  }
